...
// I2C address for LIS3DH
//#define LIS3DSH_ADDR_SEL_HIGH 0x19  // with SEL/SDO pulled up to VDD
//#define LIS3DSH_ADDR_SEL_LOW  0x18  // with SEL/SDO pulled down to VSS
...
{
	byte error;                     // 2021-22 add debug
  Restart:                        // 2021-22 add debug
	Wire.beginTransmission(I2CAddr);
	Wire.write(reg);
	Wire.write(value);
//	Wire.endTransmission();

  Serial.print("Wire.endTransmission=");  // 2021-22 add debug
  error = Wire.endTransmission();         // 2021-22 add debug
  Serial.println(error,HEX);              // 2021-22 add debug
  Serial.println();                       // 2021-22 add debug
  if (error !=0)  // use Wire.begin(2,0) have bug, first time Wire.beginTransmission (even use delay) MUST receive error "2: Received NACK on transmit of address"
    goto Restart;
}
